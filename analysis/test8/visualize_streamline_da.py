...
OUTDIR    = Path(__file__).resolve().parent / "./fig/streamline_da"
DPI       = 200
CMAP     = "viridis"
...
def main():
    ...
    with Dataset(str(NC_PATH), "r") as nc:
        ...
        u_da_var = nc.variables["optional"]
        v_da_var = nc.variables["optional2"]
        ...
        rot_uv_da_lims = vorticity_limits_over_time(u_da_var, v_da_var, dx, dy)
        (rot_uv_vmin, rot_uv_vmax) = rot_uv_lims
        (rot_uv_da_vmin, rot_uv_da_vmax) = rot_uv_da_lims
        ...
        for k in range(nt):
            ...
            u = u_var[k, :, :]                                # (ny, xi)
            v = v_var[k, :, :]                                # (eta, nx)
            u_da = u_da_var[k, :, :]                          # (ny, xi)
            v_da = v_da_var[k, :, :]                          # (eta, nx)
            u_c = 0.5 * (u + np.roll(u, 1, axis=1))           # (ny, nx)
            v_c = 0.5 * (v + np.roll(v, 1, axis=0))           # (ny, nx)
            u_da_c = 0.5 * (u_da + np.roll(u_da, 1, axis=1))  # (ny, nx)
            v_da_c = 0.5 * (v_da + np.roll(v_da, 1, axis=0))  # (ny, nx)
            speed    = np.hypot(u_c, v_c)                     # sqrt(u_c^2 + v_c^2)
            speed_da = np.hypot(u_c, v_c)                     # sqrt(u_c^2 + v_c^2)

            fig, axes = plt.subplots(
                #1, 2, figsize=(10, 5.0),
                2, 2, figsize=(10.5, 10.0),
                gridspec_kw={
                    "top": 0.88,
                    "bottom": 0.10,
                    "left": 0.03,
            ...
            axes[0, 0].set_title("Stream Line and Norm (True)", fontsize=16)
            ...
            axes[1, 0].set_title("Rotation (True)", fontsize=16)
            ...
            # Plotting streamline
            norm = matplotlib.colors.Normalize(
                vmin=float(speed_da.min()),
                vmax=float(speed_da.max())
            )
            axes[0, 1].streamplot(
                x,
                y,
            ...
            axes[0, 1].set_title("Stream Line and Norm (DA)", fontsize=16)

            # Right
            rot_uv_da = curl_from_cell_centered(u_da_c, v_da_c, dx, dy)
            im1 =axes[1, 1].imshow(
                rot_uv_da,
                origin="lower",
                extent=extent_img,
            ...
            axes[1, 1].set_title("Rotation (DA)", fontsize=16)
