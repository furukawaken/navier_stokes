...
void computeConvectionSMAC_(
    ...
    for (int jy = 0;jy < ny; ++jy){
        for (int ix = 0;ix < nx; ++ix){
            ...
            /*
            const double v_up_for_u    = 0.5*( v[index(ix,  jy+1)] + v[index(ix+1,jy+1)] );
            const double v_low_for_u   = 0.5*( v[index(ix,  jy  )] + v[index(ix+1,jy  )] );
            const double u_up_for_u    = 0.5*( u[index(ix,  jy+1)] + u[c] );
            const double u_low_for_u   = 0.5*( u[index(ix,  jy  )] + u[index(ix, jy-1)] );
            const double duvdy_u       = (v_up_for_u*u_up_for_u - v_low_for_u*u_low_for_u) * inv_dy;
            */
...
void testNavierStokesPeriodicSMAC() {
    std::cout << "testNavierStokesPeriodicSMAC" << std::endl;
    ...
    // Setting a writer
    std::string filename{"../output/test5/ns.nc"};
    ...
    Eigen::VectorXd dpdx = Eigen::VectorXd::Zero(nx * ny);
    Eigen::VectorXd dpdy = Eigen::VectorXd::Zero(nx * ny);
    ...
    for ([[maybe_unused]] int i = 0; i < max_writing_count && !shouldStop; ++i) {
        ...
        for (int it = 0; it < si; ++it) {
            ...
            = v
            + dt * (
                - positive_laplacian * v
                - dpdy
                - nonlinear_term_v
                + forcing_v
            );
            p_manager.build(u_tmp, v_tmp);

            // Updating u, v, t:
            p_manager.modifyVectorField(u_tmp, v_tmp);
            u = u_tmp;
            v = v_tmp;
            p = p_manager.getP();
            t += dt;
